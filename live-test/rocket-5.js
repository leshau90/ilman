 ...
 */
    if (members .length ==0 || !members) return 'invalid data';
    let result = {
        totalMember: 0,
        averageLevel: 0
    }
    for (let i = 0; i < members.length; i++) {
        if (!result[members[i].class]) result[members[i].class] = []
        result[members[i].class].push({
            name: members[i].name,
            level : members[i].level
        })
        result.totalMember++;
        result.averageLevel+=members[i].level;

    }
    // console.log('=====')
    // console.log(result);
    // console.log('=====')
    result.averageLevel = Math.floor (result.averageLevel/result.totalMember);
    return result;
