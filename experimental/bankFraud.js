...
function activityNotifications(expenditure, d) {
    ...
    for (let i = 0; i <= expenditure.length - d; i++) {
        // console.log('operating on:', expenditure.slice(i, d + i - 1));
        if (expenditure[d + i] > getMedian(expenditure.slice(i, d + i - 1)) * 2) {
            // console.log('adding because:', expenditure[d + i], 'is greated than ', getMedian(expenditure.slice(i, d + i - 1)) * 2)
            notif++;
        }
    }
    return notif;
